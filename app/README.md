...
字节数和 sha256 写在 `lib/src/models.dart` 的表里。源写成一张 `_mirrors` 表而不是
一个常量：目前**只有 ModelScope 一家**在发这批带内嵌字表的 ONNX（HuggingFace 上
官方的是 Paddle 格式，`SWHL/RapidOCR` 只到 v4，RapidOCR 的 GitHub Release 一个
附件都没挂），哪天我们自己往别处传一份，表里加一行就完事。

**识别开跑前会先补包。** 三条识别路径（转文档、提取文字、可搜索 PDF）都走
`Models.prepare()`：选中的语言包不在本地就当场下，进度接到各自的进度条上；下不
下来就抛 `OcrPackMissing`，界面上说清是哪种语言。这里**不退回内置模型** —— 用中
文模型认一页韩文，出来的不是「没结果」，是一整页看着像结果的错字，等用户发现时
已经没有任何线索指向「语言包没装上」。包会缺不是假想：模型目录标了不备份，从
iCloud 恢复的新设备上设置里写着韩语而文件是没有的。

要加一种语言，就是往那张表里加一行 —— 但加之前必须确认两件事：
